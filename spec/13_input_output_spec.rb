...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:incorrect_guess) {described_class.new(5,'4')}
      # remove the 'x' before running this test
      it 'is not game over' do
        expect(incorrect_guess).not_to be_game_over
      end
    end
  end
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        invalid_input = ['d','15']
        valid_input = '8'
        allow(game_loop).to receive(:player_input).and_return(invalid_input[0], invalid_input[1], valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      # remove the 'x' before running this test
      subject(:game) { described_class.new(5,'5',6)}
      it 'outputs correct phrase' do
        final_phrase = "That was hard. It took you 6 guesses!\n"
        expect { game.final_message }.to output(final_phrase).to_stdout
      end
    end
  end
end
