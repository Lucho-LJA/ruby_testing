...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        valid_input = ['+','2']
        allow(game_input).to receive(:gets).and_return(valid_input[0], valid_input[1])
      end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        valid_input = ['+','5','2']
        allow(game_input).to receive(:gets).and_return(valid_input[0], valid_input[1], valid_input[2])
      end
      ...
      it 'completes loop and displays error message twice' do
        ...
        max = 2
        error_message = "Input error! Please enter a number between #{min} or #{max}."
        expect(game_input).to receive(:puts).with(error_message).exactly(2).time
        game_input.player_input(min, max)
      end
    end
  end
  ...
  describe '#verify_input' do
    ...
    subject(:game){described_class.new(1,100)}
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        max = 2
        valid_input = 3
        expect(game.verify_input(min,max,  valid_input)).to be_nil
      end
    end
  end
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      subject(:game_3){ described_class.new(100,600) }
      it 'returns 9' do
        max = game_3.maximum_guesses
        expect(max).to eq(9)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    subject(:game_5) { described_class.new(1, 10) }
    ...
    before do
      ...
      allow(game_5).to receive(:display_guess).with(binary_search)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect {game_5.display_turn_order(binary_search)}.to change {game_5.instance_variable_get(:@guess_count)}.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_search).to receive(:make_guess)
      game_5.display_turn_order(binary_search)
